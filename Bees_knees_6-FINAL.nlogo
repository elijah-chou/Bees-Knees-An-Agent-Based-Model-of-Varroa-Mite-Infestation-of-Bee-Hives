...
to go
  ...
  ask hives [
    ...
    if ticks mod (th_reapplication) = 0 [
...
to update-goal
  ...
  [
    ...
    set goal one-of patches in-radius 50 with [flower? = true]
    face goal
    set returning-to-hive? false
    set add-food? false
  ...
  [
    ...
    ifelse random-float 1 < 0.3 [set goal one-of patches in-radius 50 with [(flower? = true)]  set returning-to-hive? false]
...
to runge
  ...
  ask patch 0 0 [
    sprout-funcs 1 [
      ...
      ifelse [th-oil] of hive this > th-threshold
        [set th 1]
        [set th 0]
      hide-turtle
    ]
...
to revert
   ask bees with [treated? = true][
    if t-oil < th-threshold [set treated? false]
  ]
end

to lose-oil
  if th-oil <= 100 [if (ticks mod 6) = 0 [set th-oil (th-oil * 0.82)]
  ]
  ask link-neighbors with [([link-length] of one-of my-out-links < 1)] [if t-oil <= 100 [if (ticks mod 6) = 0 [set t-oil (t-oil * 0.82)]
  ]]
   if sp-oil <= 100 [if (ticks mod 6) = 0 [set sp-oil (sp-oil * 0.82)]
  ]
  if sp-oil < sp-threshold [set protected? false]
...
@#$#@#$#@
<experiments>
  <experiment name="Vary_Thyme_Application" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>(ticks = 3000) or (count bees = 0)</exitCondition>
    <metric>count bees with [infested? = true]</metric>
    <metric>count bees with [sick? = true]</metric>
    <metric>count bees</metric>
    <metric>sum [mite-1] of hives</metric>
    <metric>sum [mite-2] of hives</metric>
    ...
    <steppedValueSet variable="th_reapplication" first="10" step="10" last="1000"/>
  ...
  <experiment name="Vary_Flower" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>(ticks = 1500) or (count bees = 0)</exitCondition>
    <metric>count bees with [infested? = true]</metric>
    <metric>count bees with [sick? = true]</metric>
    <metric>count bees</metric>
    <metric>sum [mite-1] of hives</metric>
    <metric>sum [mite-2] of hives</metric>
    <enumeratedValueSet variable="sp_reapplication">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number_of_bees">
      <value value="35"/>
    ...
    <steppedValueSet variable="number_of_flowers" first="5" step="5" last="50"/>
    <enumeratedValueSet variable="carrying_capacity">
      <value value="200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="th-threshold">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="sp-threshold">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number_of_hives">
      <value value="1"/>
    ...
    <enumeratedValueSet variable="th_reapplication">
      <value value="990"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="add-spearmint?">
      <value value="false"/>
  ...
  <experiment name="Vary_Thyme_Spearmint_Application" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>(ticks = 1500) or (count bees = 0)</exitCondition>
    <metric>count bees with [infested? = true]</metric>
