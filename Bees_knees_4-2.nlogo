...
hives-own [food infestation-rate percent-infested number-infested-eggs number-uninfested-eggs sp-oil th-oil mite-count pop-count protected?]
...
to go
  ...
  ask hives [
    ...
    replenish-energy
    set mite-count (round (mite-count * 0.99))
    set pop-count count my-in-links
    set label mite-count
  ]
...
to produce-offspring
  ...
  if count [link-neighbors] of my-hive < 100 and random-float 1 < 0.5 [
    ...
    set food 0
    if infested? = false [
      hatch-bees food-count [ create-link-to my-hive set age random-poisson healthy-age set energy healthy-energy update-goal]
    ]
    if infested? = true [
      ifelse random-float 1 < 0.55
      [ hatch-bees food-count [ create-link-to my-hive set sick? true set age random-poisson sick-age set energy sick-energy update-goal] ]
      [ hatch-bees food-count [ create-link-to my-hive set sick? false set age random-poisson healthy-age set energy healthy-energy update-goal ] ]
    ]
    if sick? = true [
      hatch-bees food-count [ create-link-to my-hive set age random-poisson sick-age set energy sick-energy ]
    ]
  ]
end
...
to birth-larva
  ...
  if food > 10 [
    ...
    ifelse protected? = true [
      let num-healthy-bees (total-bees-produced * 0.843)
      let num-sick-bees (total-bees-produced - num-healthy-bees)
    ]
    [
      ifelse mite-count < 400 [ set infestation-rate mite-count / 400 ]
      [ set infestation-rate 1]
      let num-sick-bees (total-bees-produced * infestation-rate)
      let num-healthy-bees (total-bees-produced - num-sick-bees)
    ]
    ...
    set food (food - 10)
    set mite-count (2 * mite-count * (1 - (mite-count / 1000)))
  ]
end
