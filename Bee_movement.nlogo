...
;patches-own [polli?]
patches-own [infectedPatch?]

bees-own [fuel infected? dist-to-goal returning-to-hive? goal]
...
to setup
  ...
  ;flower setup
  ask n-of 3 patches [ set pcolor red ]
  ask patches with [pcolor = red] [ask neighbors4 [set pcolor red]]
  ask n-of 3 patches [ set pcolor blue ]
  ask patches with [pcolor = blue] [ask neighbors4 [set pcolor blue]]
  ask n-of 3 patches [ set pcolor magenta ]
  ask patches with [pcolor = magenta] [ask neighbors4 [set pcolor magenta]]
  ...
  create-hives 1 [set color orange setxy 0 0 set size 3]
  create-bees 5 [set color yellow setxy random-xcor random-ycor set dist-to-goal 0 set returning-to-hive? false]
  ask bees [ create-link-to hive 0 update-goal]
  ask links [hide-link]
...
to update-goal
  ifelse [link-length] of one-of my-out-links < 1 [
    let found-flower-patches (patches with [pcolor != 68] in-radius 32)
  ...
  [
    if any? my-in-links [ ask one-of my-in-links [die] ]
...
to fly-to-hive
  set heading [link-heading] of one-of my-links
  fd 1
end
@#$#@#$#@
GRAPHICS-WINDOW
