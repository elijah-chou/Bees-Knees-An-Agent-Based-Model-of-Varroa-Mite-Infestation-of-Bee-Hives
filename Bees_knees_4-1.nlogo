...
globals [healthy-age sick-age healthy-energy sick-energy] ;initiated in setup

patches-own [flower? infestedPatch? time]

bees-own [infested? sick? treated? protected? dist-to-goal returning-to-hive? goal energy add-energy? add-food? my-hive age food s-oil t-oil]

hives-own [food percent-infested number-infested-eggs number-uninfested-eggs sp-oil th-oil]
...
to setup
  ...
  set food 0 set number-infested-eggs 0 set number-uninfested-eggs 0]
...
to go
  ...
  ask hives [
    update-infested-rate
    get-food
      ;birth-larva (temporarily replaced with produce-offspring function)
    ;set-energy (replaced with replenish-energy)
    replenish-energy
    lose-oil

  ]
  ask hives with [(sqrt ( (xcor - mouse-xcor) ^ 2 + (ycor - mouse-ycor) ^ 2) ) < 1] [
    drop-thyme
    spearmint
    lemongrass
  ]
...
to update-goal
  ...
  [
    ...
    set returning-to-hive? false
    set add-food? false
    produce-offspring ;bees are asked to produce offspring after returning to hive
  ]
  [
...
to set-energy ;replaced with replenish-energy
;state based
    if ([link-length] of one-of my-out-links < 1) and (infested? = false) [ set energy 100]
    if ([link-length] of one-of my-out-links < 1) and (infested? = true) [ set energy 50 ]
    ifelse show-energy?
    [ set label energy ]
    [set label " "]
end

to update-infested-rate
  ifelse count link-neighbors > 0 [
    set percent-infested  ( count link-neighbors with [infested? = true] ) / ( count link-neighbors ) set label precision percent-infested 3
    set number-infested-eggs ( (number_of_eggs * percent-infested ) )
    set number-uninfested-eggs ( number_of_eggs - ( (number_of_eggs * percent-infested) ))
  ]
  [ set percent-infested 0 ]
end

to get-food ;noticed that this also takes food away from bees that do not belong to this hive
  set food ( food +  3 * (count bees in-radius 3 with [ add-food? = true ])  )
  ask bees in-radius 3 with [ add-food? = true ] [ set add-food? false ]
end
...
to produce-offspring
  ;command for individual bees to produce offspring, proportionate to the amount of food it brings back
  if count [link-neighbors] of my-hive < 100 and random-float 1 < 0.5 [
    let food-count food
    set food 0
    if infested? = false [
      hatch-bees food-count [ create-link-to my-hive set age healthy-age set energy healthy-energy update-goal]
    ]
    if infested? = true [
      ifelse random-float 1 < 0.55
      [ hatch-bees food-count [ create-link-to my-hive set sick? true set age sick-age set energy sick-energy update-goal] ]
      [ hatch-bees food-count [ create-link-to my-hive set sick? false set age healthy-age set energy healthy-energy update-goal ] ]
    ]
    if sick? = true
     [ifelse protected? = true
        [if infested? = false [
      hatch-bees food-count [ create-link-to my-hive set age healthy-age set energy healthy-energy update-goal set protected? false]
    ]
    if infested? = true [
      ifelse random-float 1 < 0.25
      [ hatch-bees food-count [ create-link-to my-hive set sick? true set age sick-age set energy sick-energy update-goal set protected? false] ]
      [ hatch-bees food-count [ create-link-to my-hive set sick? false set age healthy-age set energy healthy-energy update-goal set protected? false] ]
    ]]
        [hatch-bees food-count [ create-link-to my-hive set age sick-age set energy sick-energy]]
      ]
    ]
...
to birth-larva
  ...
  if food > 20  and (count link-neighbors < 100) [
    ask patch-here [
      ;sprouts infested bees
      sprout-bees one-of ([number-infested-eggs] of hives-here) [
        create-link-to one-of hives-here
        set my-hive [other-end] of one-of my-out-links
        set infested? true set color red
        set dist-to-goal 0
        set returning-to-hive? false
        set size 3 update-goal
        set age healthy-age
        set energy healthy-energy
        set food 0]

      ;sprouts uninfested bees
      sprout-bees one-of ([number-uninfested-eggs] of hives-here) [
        create-link-to one-of hives-here
        set my-hive [other-end] of one-of my-out-links
        set infested? false
        set color yellow
...
to groom
   if ([link-length] of one-of my-out-links < 1) [ if random-float 1 < 0.00001 [
    set infested? false
    ]
  ]
end
...
to lemongrass
;lemongrass masks the sents of the other oils but also has same scent as the attractant pheromone of the honeybee - could lead to other bees invading hive

end

to revert
  ask bees with [protected? = true][
    if s-oil = 0 [set protected? false]
  ]
...
to lose-oil
  if sp-oil <= 100 [ every 500 [set sp-oil sp-oil - 3]]
  if th-oil <= 100 [every 500 [set th-oil th-oil - 3]]
...
to update-color ;discuss whether to use or not
  ...
  if protected? = true [set color rgb 0 255 255]
 end
