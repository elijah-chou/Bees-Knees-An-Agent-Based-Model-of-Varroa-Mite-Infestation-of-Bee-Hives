...
patches-own [flower? infectedPatch?]

bees-own [fuel infected? dist-to-goal returning-to-hive? goal energy add-energy? add-food?]
hives-own [food percent-infected number-infected-eggs number-uninfected-eggs]
...
to setup
  ...
  ;flower setup
  setup_flowers
  ask one-of patches with [flower? = true] [set infectedPatch? true]
  ...
  create-hives number_of_hives [set color rgb 255 152 66 setxy random-xcor random-ycor set food 0 set size 5 set number-infected-eggs 0
  set number-uninfected-eggs 0]
  layout-circle hives 25

  create-bees number_of_bees [set color yellow setxy random-xcor random-ycor
    set dist-to-goal 0 set returning-to-hive? false set size 3]
  ;link bees to hives
  layout-circle bees 25
  ask bees [create-link-to one-of hives update-goal]
  ; ask links [hide-link]
...
to update-goal
  ifelse [link-length] of one-of my-out-links < 1 [
    ;code to find new flower patch and set as goal
    let found-flower-patches (patches with [flower? = true] in-radius 50)
    let flower-patch-dest one-of found-flower-patches
    set goal flower-patch-dest
    set heading (towards flower-patch-dest)
    set returning-to-hive? false
    set add-food? false
  ]
  [
    ;code to return to hive after visiting a flower patch
    set heading (towards [other-end] of one-of my-out-links)
    set returning-to-hive? true
    set add-food? true
  ]
...
to get-infected
  if [infectedPatch?] of patch-here = true [
  if random-float 1 < 0.5 [
      set infected? true
      set color red
      ask patch-here [set infectedPatch? false]
      pen-down
   ]
  ]
end
...
to update-infected-rate
  set percent-infected  ( count link-neighbors with [infected? = true] ) / ( ( count link-neighbors ) * 100 ) set percent-infected round percent-infected set label percent-infected
  set number-infected-eggs ( (number_of_eggs * percent-infected ) )
  set number-uninfected-eggs ( number_of_eggs - ((number_of_eggs * percent-infected) ))
end

to get-food
  set food ( food + 10 * ( count bees-here with [ add-food? = true ] ) )
  ask bees-here with [ add-food? = true ] [ set add-food? false ]
end

to birth-larva
  ;this messes everything uppp hhh
  if food > 20 and number-infected-eggs > 0 [
    ask patch-here [
      print "Born"
      sprout-bees 2 [create-link-to one-of hives-here set infected? true set color red  set dist-to-goal 0 set returning-to-hive? false set size 5 update-goal pen-down]
      print count bees
  ]]

  if food > 20 [
    ask patch-here [
      sprout-bees 2 [create-link-to one-of hives-here set infected? false set color yellow  set dist-to-goal 0 set returning-to-hive? false set size 3 update-goal ]
  ]]
end
@#$#@#$#@
GRAPHICS-WINDOW
