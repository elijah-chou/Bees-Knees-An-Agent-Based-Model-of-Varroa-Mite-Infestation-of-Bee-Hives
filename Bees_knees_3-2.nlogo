...
bees-own [infected? dist-to-goal returning-to-hive? goal energy add-energy? add-food? my-hive age]
...
to setup
  ...
  ;flower setup
  setup_flowers
  ask n-of starting-mite-count patches with [flower? = true] [set infectedPatch? true]
  ...
  set food 0 set number-infected-eggs 0 set number-uninfected-eggs 0]
  layout-circle hives 25

  create-bees number_of_bees [set color yellow setxy random-xcor random-ycor
    set dist-to-goal 0 set returning-to-hive? false set size 3 set energy 100 set age 100]
  ;link bees to hives
...
to update-goal
  ...
  [
    ...
    ifelse random-float 1 < 0.5 [set goal one-of patches with [(flower? = true)] in-radius 50 set returning-to-hive? false]
...
to drop-mite
  if random-float 1 < 0.0001 [
    if [flower?] of patch-here = true [
      ask patch-here [set infectedPatch? true]
    ]
    set infected? false
  ]
end
...
to set-energy
;state based
    if ([link-length] of one-of my-out-links < 1) and (infected? = false) [ set energy 100]
    if ([link-length] of one-of my-out-links < 1) and (infected? = true) [ set energy 50 ]
    ifelse show-energy?
    [ set label energy ]
    [set label " "]
end
...
to update-infected-rate
  ifelse count link-neighbors > 0 [
    set percent-infected  ( count link-neighbors with [infected? = true] ) / ( count link-neighbors ) set label precision percent-infected 3
...
to replenish-energy
  ask bees in-radius 1 [ set energy 100 ]
end

to bites-the-dust
  ifelse energy = 0 or age = 0 [die]
  [
    if random-float 1 < chance_of_death [set age age - 1]
  ]
end
...
to birth-larva
  ...
  if food > 20  and (count link-neighbors < 100) [
    ask patch-here [
      ;sprouts infected bees
      sprout-bees one-of ([number-infected-eggs] of hives-here) [create-link-to one-of hives-here
      ...
      ;sprouts uninfected bees
      sprout-bees one-of ([number-uninfected-eggs] of hives-here) [create-link-to one-of hives-here
        set my-hive [other-end] of one-of my-out-links
        set infected? false
        set color yellow
        set dist-to-goal 0
