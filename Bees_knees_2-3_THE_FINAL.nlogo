...
bees-own [fuel infected? dist-to-goal returning-to-hive? goal energy add-energy? add-food? my-hive age]
...
to setup
  ...
  create-bees number_of_bees [set color yellow setxy random-xcor random-ycor
    set dist-to-goal 0 set returning-to-hive? false set size 3 set energy 100 set age 1]
  ;link bees to hives
...
;state based
  ask bees [
    if [link-length] of one-of my-out-links < 1 [
      set energy 100
    ]
    ifelse show-energy?
    [ set label energy ]
...
to birth-larva
  ...
  if food > 20 and number-infected-eggs > 0 [
    ask patch-here [
      print "Born"
      sprout-bees 2 [create-link-to one-of hives-here set my-hive [other-end] of one-of my-out-links set infected? true set color red  set dist-to-goal 0 set returning-to-hive? false set size 5 update-goal pen-down set age 1 set energy 100]
      print count bees
  ] set food (food - 20)]

  if food > 20 [
    ask patch-here [
      sprout-bees 2 [create-link-to one-of hives-here set my-hive [other-end] of one-of my-out-links set infected? false set color yellow  set dist-to-goal 0 set returning-to-hive? false set size 3 update-goal set age 1 set energy 100]
  ] set food (food - 20)]
end
@#$#@#$#@
GRAPHICS-WINDOW
