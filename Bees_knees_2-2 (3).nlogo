...
to update-infected-rate
  ask hives [set percent-infected count link-neighbors with [infected? = true] / count link-neighbors * 100 set percent-infected round percent-infected set label percent-infected]
  set number-infected-eggs ( (number_of_eggs * percent-infected) / 100 )
  set number-uninfected-eggs ( number_of_eggs - ((number_of_eggs * percent-infected) / 100))
end

to get-food
   ask bees-here [
  if add-food? = true
    [ask hives-here [set food food + 10]
  ]]
end

to birth-larva
  ;this messes everything uppp hhh
      if food > 20 and number-infected-eggs > 0 [ ask patch-here [
   sprout-bees 2 [set infected? true set color red  set dist-to-goal 0 set returning-to-hive? false set size 3 pen-down]
  ]]
     if food > 20 [ ask patch-here [
      sprout-bees 2 [set infected? false set color yellow  set dist-to-goal 0 set returning-to-hive? false set size 3 ]
  ]]
end
@#$#@#$#@
GRAPHICS-WINDOW
